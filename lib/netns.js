...
exports.setupNNS = netns => {
    let name = netns.name;
    let vethDefault = netns.vethDefault;
    let vethNNS = netns.vethNNS;
    let ipDefault = netns.ipDefault;
    let ipNNS = netns.ipNNS;
    let netmask = netns.netmask;
    return util.exec(`ip netns del ${name}`).catch(() => {
        ...
        () => util.exec(`ip netns exec ${name} ip link set dev lo up`)
    ).then(
        // Set up a veth pair
        ...
        () => util.exec(`ip netns exec ${name} ip addr add ${ipNNS}/${netmask} dev ${vethNNS}`)
    ).then(
        () => util.exec(`ip netns exec ${name} ip link set up dev ${vethNNS}`)
    ).then(
        // Enable routing inside network namespace for packets from the outside
        () => util.exec(`ip netns exec ${name} iptables -t nat -A POSTROUTING -s ${ipDefault} -j MASQUERADE`)
    ).then(
        // Allow packets forwarding
        () => util.exec(`ip netns exec ${name} sysctl net.ipv4.ip_forward=1`)
    ).then(
        () => util.exec(`ip netns exec ${name} sysctl net.ipv4.conf.${vethNNS}.forwarding=1`)
    ).then(
        ...
        () => util.exec(`ip route add ${ipNNS}/${netmask} dev ${vethDefault}`)
    ).catch(err => {
        if (err.includes('File exists')) {
            // Route already exists, possibly from other runs. Delete and try again
            return util.exec(`ip route del ${ipNNS}/${netmask}`).then(
                () => util.exec(`ip route add ${ipNNS}/${netmask} dev ${vethDefault}`)
            );
        }
        ...
        () => util.exec(`ip netns exec ${name} ip route add ${ipDefault}/32 dev ${vethNNS}`)
    ).then(
        ...
        route => {
            ...
            return util.exec(`ip route show match ${immediateGateway}`).then(
                route => {
                    ...
                    return util.exec(`ip netns exec ${name} ip route add ${immediateNetwork} dev ${vethNNS}`);
                }
            );
        }
    );
...
function _getUnusedNetwork() {
    return util.exec('ip link show').then(list => Promise.all(
        ...
        function findNextUnused(counter) {
            let network = _intFromIP(IP_START) + (Math.abs(IP_MASK) * counter);
