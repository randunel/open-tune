...
exports.setupNNS = nns => {
    ...
    return util.exec(`ip netns del ${name}`).catch(() => {
        ...
        **/
            ...
            Promise.all([
                ...
                new Promise((resolve, reject) => {
                    ...
                    function check(count) {
                        ...
                        return util.exec(`cat /sys/class/net/${vethDefault}/operstate`).then(data => {
                            if (data.includes('up')) {
                                return resolve();
                            }
                            if (data.includes('lowerlayerdown')) {
        ...
        () => util.exec(`sysctl net.ipv4.ip_forward=1`)
    ).then(
